 ...
 * Extracted from src/modules/book-translation/utils/text.ts for server-side use.
 */
...
function normalizeVietnameseCluster(baseCluster: string, spacingMarks: string) {
  ...
  if (!baseChar) return baseCluster;
  ...
  const collectMark = (mark: string) => {
    if (BASE_COMBINING_MARKS.has(mark)) {
      if (!baseMark) baseMark = mark;
      return;
    }
    if (TONE_COMBINING_MARKS.has(mark)) {
      if (!toneMark) toneMark = mark;
      return;
    }
...
export function repairLikelyMojibake(value: string) {
  if (!LIKELY_MOJIBAKE_PATTERN.test(value)) return value;
